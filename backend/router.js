    ...
    if (req.body.firstName.length == 0 || req.body.lastName.length == 0 || req.body.email.length == 0 || req.body.phone.length == 0) {
        ...
        db.query("SELECT * FROM users WHERE email ='" + req.body.email + "';", (err, result) => {
            if (result.length > 0) {
                ...
                var url = "http://wifiticket.wifinetcom.net:8080/WIFITicketSystem2/TicketServlet?action=get_ticket_sms&pin=03904800756090620171251&nome=" + req.body.firstName + "&cognome=" + req.body.lastName + "&phone=" + req.body.phone + "&email=" + req.body.email + "&days=7";
